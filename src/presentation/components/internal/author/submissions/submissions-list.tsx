...
import { getAuthorSubmissionsAction } from '@app/presentation/actions/submission.actions';
...
export function SubmissionsList({ initialSubmissions, initialPaginationOptions }: Props) {
  ...
  const handlePaginationModelChange = async (newPaginationModel: GridPaginationModel) => {
    ...
    try {
      const result = await getAuthorSubmissionsAction(
        ['author', 'journal'],
        cursor,
            ...
            },
            {
              ...
              renderCell: (params) => (
                <GridActionsCell {...params}>
                  ...
                  />
                  {params.row.actions.status === 'DRAFT' && (
                    <GridActionsCellItem
                      key="edit"
                      showInMenu
                      icon={<EditRounded />}
                      ...
                      component={Link}
                      // @ts-expect-error Link component requires href prop but it does not exposed as a prop for some reason. Read more on https://github.com/mui/mui-x/issues/9913
                      href={`/submissions/${params.row.actions.id}/edit`}
                    />
                  )}
                </GridActionsCell>
              ),
